# note-checklists Specification

## Purpose

Support an optional task list ("Show checkboxes") on notes. Users can convert a note's text content into a checklist, add/delete/check items in both the editor and the take-note composer, and see checklist items with completion progress rendered in note cards. Checklist item text is included in note search.
